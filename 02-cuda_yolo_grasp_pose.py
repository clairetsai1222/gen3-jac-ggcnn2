...
from utils.yolo import cuda_object_detection
...
# 加载GGCNN模型并将其移动到CUDA 0
ggcnn_model = GGCNN2().to(device)
...
try:
    while stop_flag:
        ...
        # 进行目标检测
        detect_result = cuda_object_detection.ObjectDetection(detect_object=expecting_detected_object, color_image=resized_color_image, depth_image=aligned_depth_image, color_intrinsics=color_intrinsic_matrix)
        objects_dict, object_keys = detect_result.get_results()

        print("detected objects:", objects_dict)
        print("detected object keys:", object_keys)
        ...
        if len(object_keys) != 0:
            for key in object_keys:
                ...
                object_color_image = resized_color_image[int(y1):int(y2), int(x1):int(x2)]

                # 在object_color_image中用x1,y1,x2,y2绘制出方框
                cv2.rectangle(object_color_image, (x1, y1), (x2, y2), (0, 255, 0), 2)

                # 显示带有方框的object_color_image
                cv2.imshow('Object Color Image with Bounding Box', object_color_image) 

                # 使用GGCNN模型预测抓取点
                with torch.no_grad():
                    depthT = torch.from_numpy(object_depth_image.reshape(1, 1, y2-y1, x2-x1).astype(np.float32)).to(device)
                    grasp_imgs = ggcnn_model(depthT)

                q_img, ang_img, width_img = grocess_output.post_process_output(q_img=grasp_imgs[0].cpu(), cos_img=grasp_imgs[1].cpu(), sin_img=grasp_imgs[2].cpu(), width_img=grasp_imgs[3].cpu())
                ...
                for grasp_objects in grasps:
                    ...
                    # 将抓取点转换为相机坐标系
                    grasp_point_camera_frame = (rectangle_center[0] + x1, rectangle_center[1] + y2)
