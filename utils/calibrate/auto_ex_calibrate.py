...
import class_gen3_gripper_pose
...
gp = class_gen3_gripper_pose.Gen3GripperPose()
        ...
        if rec:
            ...
            if img is None:
                ...
                with open(file_save_path, 'w') as f:
                    f.write(f"{cartesian_pose.x} {cartesian_pose.y} {cartesian_pose.z} "
...
def calibrate(path):
    ...
    for img_name,file_name in zip(imgs_name,files_name):
        Ts_board_in_camera = get_Ts_board_in_camera(img_name)
        Ts_hand_in_base = get_Ts_hand_in_base(file_name)
        if isinstance(Ts_board_in_camera, int) or isinstance(Ts_hand_in_base, int):
            continue
...
if __name__ == '__main__':
    #图片所在路径
    path = f'./ex_aruco_calibration_image'
    num = input("saving numbers...\n")
    batch_save_img_and_base_pose(int(num), path)
