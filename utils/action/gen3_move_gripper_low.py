    ...
    def Goto(self, target_position):
        ...
        while True:
            try:
                ...
                # print(base_feedback.interconnect.gripper_feedback.status_flags)
                ...
                status_flags = base_feedback.interconnect.gripper_feedback.status_flags
                # If positional error is small, stop gripper
                if abs(position_error) < 1.5 or status_flags == 11:
                    position_error = 0
                    self.motorcmd.velocity = 0
                    ...
                    if self.motorcmd.velocity > 50.0:
                        self.motorcmd.velocity = 50.0
...
def get_feedback(args):
    ...
    with utilities.DeviceConnection.createTcpConnection(args) as router:
        ...
        with utilities.DeviceConnection.createUdpConnection(args) as router_real_time:
            example = GripperLowLevelExample(router, router_real_time, 2)
            position = example.motorcmd.position
            example.Cleanup()
            return position

def gripper_action(args, position):
    # Import the utilities helper module
    # import argparse
    # sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))
