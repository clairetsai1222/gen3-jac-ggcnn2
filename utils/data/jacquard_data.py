...
class JacquardDataset(GraspDatasetBase):
    ...
    def __init__(self, file_path, start=0.0, end=1.0, ds_rotate=0, **kwargs):
        ...
        pattern = '*_grasps.txt'
        graspf = list(Path(file_path).glob(f'**/{pattern}'))
        ...
        depthf = [str(f).replace("_grasps.txt", "_perfect_depth.tiff") for f in graspf]
        rgbf = [str(f).replace("_perfect_depth.tiff", "_RGB.png") for f in depthf]
